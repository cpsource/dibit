...
typedef struct rndbbs_struct_t {
  size_t key_bitlen;
  mpz_t blumint;
  mpz_t x;
  int improved;
...
int rndbbs_gen_blumint(RNDBBS *bbs, unsigned int key_bitlen);
int rndbbs_gen_x(RNDBBS *bbs);
RNDBBS *rndbbs_new();
int rndbbs_destroy(RNDBBS *bbs);
//void rndbbs_randbytes(RNDBBS *bbs, size_t nbytes);
unsigned int *rndbbs_randint(RNDBBS *bbs, unsigned int base, size_t nmemb);
